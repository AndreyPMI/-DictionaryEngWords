...
internal fun DialogWindow(
    ...
    Dialog(onDismissRequest = onClose) {
        Surface(
            ...
            Column(Modifier.padding(MaterialTheme.dimension.size8)) {
                Text(text = title)
                OutlinedTextField(
                ...
                Spacer(modifier = Modifier.height(MaterialTheme.dimension.size48))
                Row(horizontalArrangement = Arrangement.End) {
                    Button(onClick = onClose) {
                        Text(stringResource(id = R.string.dialog_cancel))
                    }
                    Spacer(modifier = Modifier.width(MaterialTheme.dimension.size64))
                    Button(onClick = {
                        onSubmit(wordState)
                    }) {
